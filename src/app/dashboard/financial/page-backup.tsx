...
export default function FinancialPage() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      await loadPendingAppointments(barbershopData.id)
  ...
  const loadPendingAppointments = async (barbershopId: string) => {
    try {
      console.log('Loading appointments for barbershop:', barbershopId)
      
      // Consulta directa para obtener citas pendientes
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const montoFinal = (parseFloat(paymentData.monto_pagado) || selectedAppointment.precio) - parseFloat(paymentData.descuento)
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          descuento_aplicado: parseFloat(paymentData.descuento),
          propina: parseFloat(paymentData.propina),
          notas_pago: paymentData.notas || null,
      ...
      // Recargar datos
      if (barbershop) {
        await loadPendingAppointments(barbershop.id)
      }
      
      alert(`Pago procesado exitosamente para ${selectedAppointment.cliente_nombre}`)
                        ...
                        <div className="flex items-center space-x-3 mb-3">
                          ...
                          {appointment.fecha && (
                            <span className="text-xs text-blue-600 bg-blue-100 px-2 py-1 rounded-full">
                              {format(new Date(appointment.fecha), 'dd/MM', { locale: es })}
                            </span>
                          )}
                        </div>
                          ...
                          <div className="space-y-1">
                            <p className="text-sm text-gray-600 flex items-center">
                              ...
                              <span className="font-medium">
                                {format(new Date(appointment.fecha), 'EEEE, dd MMMM yyyy', { locale: es })}
                              </span>
                            </p>
                            <p className="text-sm text-gray-600 flex items-center">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setPaymentData({ ...paymentData, metodo_pago: e.target.value as any })}
